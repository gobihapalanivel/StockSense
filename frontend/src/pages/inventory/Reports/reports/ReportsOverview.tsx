...
export default function ReportsOverview({ onViewChange }: { onViewChange: (view: ViewState) => void }) {
  ...
  const getRoleBadge = (role: string) => {
    if (role === 'ADMIN') {
      return <span className="px-2.5 py-1 text-[10px] font-bold bg-[#ede9fe] text-[#7c3aed] rounded-full">Admin</span>;
    }
    if (role === 'INVENTORY_MANAGER') {
      return <span className="px-2.5 py-1 text-[10px] font-bold bg-[#dbeafe] text-[#1d4ed8] rounded-full">Manager</span>;
    }
    return <span className="px-2.5 py-1 text-[10px] font-bold bg-slate-100 text-slate-600 rounded-full">{role}</span>;
  };
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Generated By</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="p-8 text-center text-slate-500">No reports generated recently.</td>
                </tr>
              ) : (
                reports.map((report) => (
                  <tr key={report.id}>
                    ...
                    <td className="p-4">{getRoleBadge(report.createdByRole)}</td>
